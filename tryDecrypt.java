 ...
 * @author Ahmad Raf
 */
public class tryDecrypt extends SubKey{
    ...
    void DecryptMessage(String message)throws Exception{
        binnaryValueMessage = textToBinnary(message);
        ...
        for(int x=0; x<messageGroup.length; x++){
            ...
            for(int i=0; i<16; i++){
                BitSelection(i);
                XOR(subKey[16 -(i+1)].toCharArray(), binnaryBitSelect, binnaryXORE);
                Grouping();
                SBOX();
                Permutation();
                XOR(l[i], permutate, r[i+1]);
                System.out.println("\nHasil Dari Round " +(i+1));
                System.out.println("L1" + String.valueOf(l[i+1]));
                System.out.println("R1" + String.valueOf(r[i+1]));
